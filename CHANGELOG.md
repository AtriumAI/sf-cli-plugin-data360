...
- **`CrudUpdateCommand`** — an empty resource id now raises a clear error instead of building a `//` path that 404s opaquely (oclif's `required: true` accepts an empty string).
